...
impl FoolsMate {
    ...
    fn need_to_change(&self) -> bool {
        let velocity: Vector = self.uav_heading - self.enemy_heading;
        let starting_pos: Vector = Vector::from_point(self.uav_point);
        let time: f32 = -starting_pos.dot(velocity) / velocity.get_magnitude().powi(2);
        let closest_pos: Point = Point::from_vector(starting_pos + velocity * time);
        self.sphere
            .within_inner_sphere(closest_pos, self.enemy_point)
    }
...
impl Algorithm for FoolsMate {
    fn evade(&mut self) -> LinkedList<Node> {
        ...
        {
            self.rotate_space();
            if self.uav_within_sector(FoolsMate::THETA) && self.need_to_change() {
                self.evade();
            } else if self
                .sphere
        ...
        self.revert_space();
        let mut new_path: LinkedList<Node> = LinkedList::new();
        for node in self.path.iter() {
            let temp: Node = Node::new(node.to_location(self.ref_point), false);
...
mod tests {
    ...
    fn test_change_rotation() {
        ...
        fm.rotate_space();
        assert_eq!(fm.need_to_change(), true)
    }
}
